...
module.exports = {
    ...
    update: async function(req, res, next) {
        try {
            ...
            res.render('./pages/view', { book: updatedBook , counter: counter, user: req.user ? req.user : false })
        } catch (error) {
            next(new ServerError(error))
        }
    },
    ...
    get_pageIndex: async function(req, res, next) {
        try {
            let books = await Book.getAllBooks()
            if(books) {
                res.render('./pages/index', { books: books, user: req.user ? req.user : false }, (err, html) => {
    ...
    get_pageCreate: function(req, res) {
        res.render('./pages/create', {message: '', user: req.user ? req.user : false})
    },
    get_pageView: async function(req, res, next) {
        try {
            let counter = await postIncrCounter(req.id)
            res.render('./pages/view', { book: req.matchBook, counter, user: req.user ? req.user : false })
        } catch (error) {
            next(new ServerError(error))
        }
    },
    get_pageUpdate: function(req, res) {
        res.render('./pages/update', {book: req.matchBook, user: req.user ? req.user : false})
    }
}