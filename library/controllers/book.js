...
async function getCounter(id) {
    fetch(`http://localhost:3001/counter/${id}`, {
            method: 'GET',
            headers: {
                'Origin': 'http://localhost:3000'
            }
    })
    .catch(err => console.log(err, 'ответ на GET провал'))
}
// запрос на удаление счетчика для книги после удаления самой книги
async function postDelCounter(id) {
    fetch(`http://localhost:3001/counter/${id}/delete`, {
            method: 'POST',
            headers: {
                'Origin': 'http://localhost:3000'
            }
    })
    .catch(err => console.log(err, 'ответ на POST провал'))
}
...
module.exports = {
    ...
    update: async function(req, res, next) {
        ...
        res.render('./pages/view', { book: updatedBook })
